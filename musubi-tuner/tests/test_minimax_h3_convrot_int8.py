...
def test_prequantized_checkpoint_with_merge_hook_requantizes(tmp_path):
    config = _convrot_config()
    bf16_path = tmp_path / "tiny_bf16.safetensors"
    prequant_path = tmp_path / "tiny_int8_convrot.safetensors"
    bf16_state = _save_tiny_bf16_checkpoint(bf16_path, config)
    ...
    state_dict = _make_quantizer().load_and_quantize(
        [str(prequant_path)],
        calc_device=None,
        weight_hook=lambda key, value, keep_on_calc_device=False: value,
    )

    assert state_dict["blocks.0.mlp.fc1.weight"].dtype is torch.int8
    assert state_dict["blocks.0.mlp.fc1.scale_weight"].dtype is torch.float32
