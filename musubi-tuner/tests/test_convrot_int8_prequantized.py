...
def test_conversion_merges_into_prequantized_weights_and_requantizes(tmp_path):
    torch.manual_seed(0)
    original = torch.randn(8, 16)
    quantized, scale = quantize_int8_convrot_weight(original, 4)
    path = _save(
        tmp_path / "artifact.safetensors",
        {
            "linear.weight": quantized,
            "linear.weight_scale": scale,
            "linear.comfy_quant": _payload(4),
        },
    )
    delta = torch.full_like(original, 0.05)

    state_dict, _quantizer = _load_prequantized(
        path,
        weight_hook=lambda key, value, keep_on_calc_device=False: value + delta,
    )

    dequantized_base = dequantize_int8_convrot_weight(quantized, scale, 4)
    expected_quantized, expected_scale = quantize_int8_convrot_weight(dequantized_base + delta, 4)
    assert torch.equal(state_dict["linear.weight"], expected_quantized)
    assert torch.equal(state_dict["linear.scale_weight"], expected_scale)
