...
try:
    ...
    # Musubi does not require optimum-quanto. An empty tuple keeps isinstance
    # checks false while retaining support when the optional package is present.
    QBytesTensor = ()
...
def copy_stochastic_bf16(target: torch.Tensor, source: torch.Tensor):
    ...
    for start in range(0, source_flat.numel(), chunk_size):
        source_chunk = source_flat[start : start + chunk_size]
        result = torch.randint_like(source_chunk, dtype=torch.int32, low=0, high=(1 << 16))
        result.add_(source_chunk.view(dtype=torch.int32))
        result.bitwise_and_(-65536)  # -65536 = FFFF0000 as a signed int32
        target_flat[start : start + chunk_size].copy_(result.view(dtype=torch.float32))
...
class Auto8bitTensor:
    def __init__(self, data: Tensor, *args, **kwargs):
        if isinstance(data, dict):  # Add constructor from state dict
            ...
            self.quantized = torch.empty_like(data, dtype=torch.int8)
            data_flat = data.reshape(-1)
            quantized_flat = self.quantized.reshape(-1)
            chunk_size = 4 * 1024 * 1024
            for start in range(0, data_flat.numel(), chunk_size):
                work = data_flat[start : start + chunk_size].div(scale)
                work.round_().clamp_(-127, 127)
                quantized_flat[start : start + chunk_size].copy_(work)
            self.scale = scale
            self.orig_dtype = data.dtype

    def dequantize(self) -> Tensor:
        result = torch.empty_like(self.quantized, dtype=torch.float32)
        result_flat = result.reshape(-1)
        quantized_flat = self.quantized.reshape(-1)
        chunk_size = 4 * 1024 * 1024
        for start in range(0, quantized_flat.numel(), chunk_size):
            result_flat[start : start + chunk_size].copy_(quantized_flat[start : start + chunk_size])
            result_flat[start : start + chunk_size].mul_(self.scale)
        return result
...
def stochastic_grad_accummulation(param, offload_to_cpu: bool = False):
    if param.grad is None:
        return
    target_device = torch.device("cpu") if offload_to_cpu else param.grad.device
    if hasattr(param, "_accum_grad"):
        grad_fp32 = param._accum_grad.clone().to(torch.float32)
        grad_fp32.add_(param.grad.to(device=target_device, dtype=torch.float32))
        copy_stochastic(param._accum_grad, grad_fp32)
        del grad_fp32
        del param.grad
    else:
        param._accum_grad = param.grad.detach().to(device=target_device, copy=True)
        del param.grad
