                ...
                for key in tqdm(keys, desc=f"Loading {os.path.basename(model_file)}", unit="key"):
                    ...
                    if key.endswith(COMFY_WEIGHT_SCALE_SUFFIX):
                        ...
                        if weight_hook is not None:
                            continue  # merged and requantized together with the sibling weight
                    ...
                    if module_path is not None and module_path in prequantized_groupsizes:
                        ...
                        if weight_hook is not None:
                            scale_key = module_path + COMFY_WEIGHT_SCALE_SUFFIX
                            scale = f.get_tensor(scale_key)
                            if scale.dtype is not torch.float32:
                                raise ValueError(f"Pre-quantized ConvRot scale {scale_key} must be F32, got {scale.dtype}")
                            expected_scale_shape = (value.shape[0], 1)
                            if tuple(scale.shape) != expected_scale_shape:
                                raise ValueError(
                                    f"Pre-quantized ConvRot layer {module_path}: scale shape must be"
                                    f" {expected_scale_shape}, got {tuple(scale.shape)}"
                                )
                            merge_device = torch.device(calc_device) if calc_device is not None else value.device
                            merged_weight = dequantize_int8_convrot_weight(
                                value.to(merge_device), scale.to(merge_device), groupsize
                            )
                            merged_weight = weight_hook(key, merged_weight, keep_on_calc_device=True)
                            value, scale = quantize_int8_convrot_weight(merged_weight, groupsize)
                            state_dict[key] = value.to(passthrough_device)
                            state_dict[module_path + ".scale_weight"] = scale.to(passthrough_device)
                        else:
                            state_dict[key] = value.to(passthrough_device)
