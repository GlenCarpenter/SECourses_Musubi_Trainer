"""Vision preprocessing for the VideoScore2 inferencer (Qwen2.5-VL variant).

Vendored VERBATIM from qwen_vl_utils (github.com/QwenLM/Qwen2.5-VL,
qwen-vl-utils/src/qwen_vl_utils/vision_process.py) -- the SAME copy the
TIGER-Lab/VideoScore2 model card imports. The scorer prefers
the installed qwen_vl_utils (so it is byte-identical to the original when present); this
local copy is the self-contained fallback so the vendor works without the external
package. The smart-resize / frame-sampling math shapes the model input directly, so it is
kept faithful. decord / torchcodec are optional backends (falls back to the torchvision
reader); requests is only touched for http(s) image paths, which the reward never uses. Do
NOT reformat the resize / pixel math.

Difference from the Qwen2-VL vision_process vendored under videoreward: this is the
Qwen2.5-VL revision (patch-size-based factor, MODEL_SEQ_LEN token budget, torchcodec
backend, process_vision_info(..., return_video_kwargs=...) returning the per-video fps
kwargs). VideoScore2 is a Qwen2.5-VL checkpoint, so this revision is required for input
parity.
"""
...
def fetch_video(
    ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False, return_video_metadata: bool = False
