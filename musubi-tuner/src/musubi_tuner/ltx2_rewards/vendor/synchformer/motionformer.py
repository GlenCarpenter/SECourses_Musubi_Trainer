...
from ._timm_compat import trunc_normal_
from .video_model_builder import VisionTransformer


class _AttrDict(dict):
    """dict with attribute access + ``.get`` (omegaconf-style) used to hold the
    MotionFormer ``divided_224_16x4`` config inline (so omegaconf is not a dep)."""
...
def _make_mformer_cfg() -> _AttrDict:
    """Inlined ``divided_224_16x4.yaml`` (Motionformer SSv2 config) + the patches the
    upstream code applies after loading it. Verbatim values from the vendored YAML.
    Only the fields read by ``VisionTransformer`` / ``MotionFormer`` are included.
    """
    cfg = _AttrDict(
        TRAIN=_AttrDict(DATASET="Ssv2"),
        DATA=_AttrDict(TRAIN_CROP_SIZE=224),
        MODEL=_AttrDict(NUM_CLASSES=174),
        VIT=_AttrDict(
            PATCH_SIZE=16,
            PATCH_SIZE_TEMP=2,
            CHANNELS=3,
            ...
            HEAD_ACT="tanh",
            ATTN_LAYER="divided",
            # patches applied by upstream MotionFormer.__init__ after loading the cfg:
            ATTN_DROPOUT=0.0,
            POS_EMBED="separate",  # divided_224_16x4.yaml -> 'separate'
...
class MotionFormer(VisionTransformer):
    ...
    2. Builds the cfg (inlined `divided_224_16x4`, no download) and patches it if needed.
    ...
    def __init__(
        ...
        # The Synchformer state dict carries the vfeat_extractor weights, so MotionFormer is
        # always built from scratch here (ckpt_path is None) and the weights are loaded later
        # into the parent Synchformer module. The upstream ckpt auto-download path is dropped.
        if self.ckpt_path is not None:
            raise NotImplementedError(
                "Vendored MotionFormer is inference-only and loads weights via the parent "
                "Synchformer state_dict; per-extractor ckpt_path is not supported."
            )

        mformer_cfg = _make_mformer_cfg()
