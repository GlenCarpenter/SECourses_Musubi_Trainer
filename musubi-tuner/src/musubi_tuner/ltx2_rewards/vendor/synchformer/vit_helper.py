...
from ._timm_compat import to_2tuple
...
class DividedSpaceTimeBlock(nn.Module):
    def __init__(
        ...
        self.timeattn = DividedAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, seq_len=196, num_frames=8, approx="none", num_landmarks=128, tok_mask: torch.Tensor = None):
        time_output = self.timeattn(self.norm3(x), self.einops_from_time, self.einops_to_time, n=seq_len, tok_mask=tok_mask)
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=224, temporal_resolution=4, in_chans=3, patch_size=16, z_block_size=2, embed_dim=768, flatten=True):
        super().__init__()
        ...
        ###
        self.proj = nn.Conv3d(
            in_chans, embed_dim, kernel_size=(z_block_size, patch_size, patch_size), stride=(z_block_size, patch_size, patch_size)
        )
