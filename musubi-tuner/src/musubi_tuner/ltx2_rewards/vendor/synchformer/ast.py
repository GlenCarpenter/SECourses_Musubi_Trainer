...
class AST(torch.nn.Module):
        ...
        if self.extract_features:
            ...
            if factorize_freq_time:
                ...
                if agg_freq_module == "TransformerEncoderLayer":
                    ...
                    self.freq_attn_agg = AveragePooling(avg_pattern="BS D f t -> BS D t", then_permute_pattern="BS D t -> BS t D")
                # time aggreration
    ...
    def forward(self, x: torch.Tensor, for_loop: bool = False, cont_mask: torch.Tensor = None, **ast_kwargs) -> torch.Tensor:
        """
        ...
        if for_loop:
            ...
            x = torch.cat([self.forward_segments(x[:, s], orig_shape_s, **ast_kwargs).unsqueeze(1) for s in range(S)], dim=1)
        else:
            orig_shape = (B, S, T, F)
            x = x.view(B * S, T, F)
