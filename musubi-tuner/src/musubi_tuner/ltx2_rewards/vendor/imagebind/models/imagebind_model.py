...
#   - ``imagebind_huge`` no longer auto-downloads weights from fbaipublicfiles; it
#     loads from an explicit ``ckpt_path`` (falls back to the IMAGEBIND_CKPT env var),
#     and raises FileNotFoundError if the checkpoint is absent. ``map_location="cpu"``
#     so the caller controls device placement.
...
def imagebind_huge(pretrained=False, ckpt_path=None):
    """Construct the ImageBind "huge" model (out_embed_dim=1024).

    Inference-only deviation from upstream: when ``pretrained`` is True the weights
    are loaded from ``ckpt_path`` (or the ``IMAGEBIND_CKPT`` env var, default
    ``checkpoints/imagebind_huge.pth``). There is NO network download fallback; a
    ...
    if pretrained:
        ...
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(
                f"ImageBind checkpoint not found at {ckpt_path!r}. Pass an explicit "
                "checkpoint_path (the vendored copy does not auto-download)."
            )
