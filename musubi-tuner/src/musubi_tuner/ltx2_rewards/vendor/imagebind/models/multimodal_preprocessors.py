...
#   - ``from imagebind.models.helpers import ...`` -> relative ``from .helpers import ...``
#   - ``from timm.models.layers import trunc_normal_`` -> inlined copy in
#     ``.._timm_compat`` (verbatim from timm; init-time only).
#   - SimpleTokenizer no longer depends on iopath's g_pathmgr; the bpe vocab is opened
#     with a plain ``gzip.open`` (the file is a normal path on disk).
...
class SimpleTokenizer(object):
    def __init__(self, bpe_path: str, context_length=77):
        ...
        # Inference-only deviation from upstream: read the bpe vocab with a plain
        # ``gzip.open`` instead of iopath's ``g_pathmgr`` (the path is always local).
