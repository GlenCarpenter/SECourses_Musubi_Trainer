"""Vision preprocessing for the VideoReward (VideoAlign) inferencer.

Vendored VERBATIM from github.com/KwaiVGI/VideoAlign
(``flow_grpo/videoalign/vision_process.py``), itself adapted from
github.com/kq-chen/qwen-vl-utils. The VideoAlign inferencer uses THIS local copy
(not ``qwen_vl_utils``) in ``inference.py`` so we keep it byte-for-byte to stay
score-identical: the smart-resize / frame-sampling math directly shapes the model
input. ``decord`` is optional (falls back to the torchvision reader); ``requests``
is only touched for http(s) image paths, which the reward never uses.
"""
