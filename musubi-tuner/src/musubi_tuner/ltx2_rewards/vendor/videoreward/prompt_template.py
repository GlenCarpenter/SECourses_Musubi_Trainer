"""Prompt templates for the VideoReward (VideoAlign) reward model.

Copied VERBATIM from github.com/KwaiVGI/VideoAlign
(``flow_grpo/videoalign/prompt_template.py``). These strings are part of the model
contract: the exact whitespace (markdown trailing-space hard breaks after some
headers/sentences and the special ``<|VQ_reward|>`` / ``<|MQ_reward|>`` /
``<|TA_reward|>`` markers plus the trailing ``END`` lines) drives tokenization and
the special-token pooling, so they are reproduced byte-for-byte. The VideoReward
checkpoint uses ``prompt_template_type = "detailed_special"`` ->
``DETAILED_PROMPT_WITH_SPECIAL_TOKEN``. Do NOT reformat this file.
"""
...
def build_prompt(prompt, dimension, template_type):
    ...
    if template_type == "none":
        ...
        return SIMPLE_PROMPT.format(dimension_name=dimension_name, dimension_description=dimension_description, text_prompt=prompt)
