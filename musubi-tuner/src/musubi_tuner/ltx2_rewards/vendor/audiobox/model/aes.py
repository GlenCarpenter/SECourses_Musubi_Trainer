...
# Vendored inference-only copy of ``audiobox_aesthetics.model.aes`` (v0.0.4).
# The only change from upstream is dropping the ``huggingface_hub.PyTorchModelHubMixin``
# base class (and its ``repo_url`` / ``pipeline_tag`` / ``license`` kwargs): the mixin
# only adds ``from_pretrained`` / ``save_pretrained`` helpers used by the HF download
# path, which the reward plugin never uses (it always loads a local checkpoint via
# ``load_state_dict``). Removing it leaves the ``nn.Module`` weights and ``forward``
# byte-for-byte identical, so scores are unchanged.
...
@dataclass(eq=False)
class AesMultiOutput(nn.Module):
    ...
    def __post_init__(self):
        ...
        if self.use_weighted_layer_sum:
            self.layer_weights = nn.ParameterDict(
                {x: torch.nn.Parameter(torch.ones(self.nth_layer) / (self.nth_layer)) for x in self.axes_name}
            )
    ...
    def forward(self, batch):
        ...
        with (
            ...
            for name in self.axes_name:
                if self.use_weighted_layer_sum:
                    norm_weights = torch.nn.functional.softmax(self.layer_weights[name], dim=-1)  # L
                ...
                audio_embed = (audio_embed * embed_mask).sum(dim=1) / embed_mask.sum(dim=1).clamp(min=1)
